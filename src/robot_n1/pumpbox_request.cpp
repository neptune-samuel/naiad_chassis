...
bool parse_pumpbox_device_state(sacp::AttributeArray const &attrs, 
   DeviceIndex & index, naiad::chassis::MsgPumpBoxState &device)
{
    ...
    DeviceIndex index, naiad::chassis::MsgDeviceInfo & info)
{
    ...
    bool ret1 = parse_pumpbox_device_brief(result->attributes, index, info.device_brief);
    bool ret2 = parse_pumpbox_admin_status(result->attributes, index, info.admin_status);
